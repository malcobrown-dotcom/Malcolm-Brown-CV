# Malcolm Brown — Career Portfolio

📍 Singapore | 📧 [malcobrown@gmail.com](mailto:malcobrown@gmail.com) | 📱 +65 8288-1557 | [LinkedIn](https://www.linkedin.com/in/malcolm-brown-2531529)

\---

## Senior Technology Business Management, PMO \& Transformation Leader

I am a senior technology business management, PMO, people and culture, and transformation leader with more than 25 years of experience across financial services, insurance and global technology organisations.

At the heart of my work is problem-solving. I am most effective when a process is fragmented, data is missing, responsibilities are unclear, or an organisation knows something needs to change but has not yet worked out how to make it happen.

I start by understanding the real problem rather than accepting the existing process. I bring together incomplete data, operational knowledge and different stakeholder perspectives to create a clear view of what is happening and what needs to change. Where necessary, I build a practical manual solution first, prove that it works, and then standardise, simplify or automate it using technology.

My approach combines analysis, persistence, curiosity and relationships. I identify useful ideas and practices from other teams, industries or parts of an organisation, adapt them to the new context and build the consensus needed to implement them. I am comfortable challenging established ways of working, but I do so by involving people, listening carefully and bringing stakeholders with me.

\---

## 📑 Quick Navigation

* [Career Impact at a Glance](#career-impact-at-a-glance)
* [AI-Enabled Applications, Agents \& Workflows](#ai-enabled-applications-agents-and-workflows)
* [Career Experience](#career-experience)
* [Core Skills \& Capabilities](#core-skills-and-capabilities)
* [Education \& Professional Development](#education-and-professional-development)
* [About Me](#about-me)

\---

<a id="career-impact-at-a-glance"></a>

## 📊 Career Impact at a Glance

* Delivered more than **USD 30M in technology efficiencies within eight months** through an eight-pillar Technology APEX programme.
* Developed an enterprise workforce plan covering approximately **3,800 employees across 14+ locations**.
* Created KPI, metrics and organisational-health frameworks for a **4,000+ person retail technology organisation**.
* Directed governance across a **USD 60M+ project and BAU portfolio**.
* Managed a **USD 40M annual technology budget** and **USD 80M application cost base**.
* Built technology cost, cloud cost, TCOA, cost-to-serve and productivity methodologies across banking and insurance technology environments.
* Established a Vendor Management Office, preferred-vendor arrangements and outcome-based managed-services governance.
* Built and led regional teams, including **nine direct reports across four countries**.
* Designed and delivered enterprise workforce, learning, mentoring, culture, engagement and future-skills programmes.
* Built bespoke AI-enabled applications, reusable agents, specialist personas and structured workflows for defined professional and personal outcomes.

\---

<a id="ai-enabled-applications-agents-and-workflows"></a>

## 🤖 AI-Enabled Applications, Agents \& Workflows

I use generative AI as a practical design and delivery tool, combining business expertise, structured prompting, iterative testing and AI-assisted development to create customised applications and workflows for specific outcomes.

* Designed and iteratively built **bespoke AI-enabled applications**, translating user needs into requirements, workflows, data structures, interfaces and test scenarios.
* Developed **Career Radar**, a configurable desktop application supporting structured job discovery, role assessment, career evidence management and application tracking.
* Built reusable **AI agents and specialist personas** for defined research, analysis, advisory and workflow tasks.
* Created structured prompt frameworks incorporating context, constraints, source-of-truth rules, evaluation criteria, output formats and quality-control checks.
* Designed multi-stage AI workflows covering research, assessment, drafting, review, verification and final output.
* Used persona design to produce more consistent specialist behaviour across recruitment, career advisory, investment research, travel planning and business analysis use cases.
* Built reusable prompt libraries and templates to improve consistency, reduce repeated effort and make complex workflows easier to use.
* Led AI-tool adoption and capability-building activity across Technology and Operations teams.
* Completed an MBA dissertation examining how AI may reshape People & Culture in financial-services technology organisations.

\---

<a id="career-experience"></a>

## 🏢 Career Experience

### Prudential, Singapore — Aug 2024 to Present

**Head of Business Management, T\&O Service Delivery Office**

Partnered directly with senior Technology and Operations leadership, including the COO, CIO and CTOO, to strengthen strategy execution, workforce planning, cost transparency, vendor governance, organisational capability and operating discipline.

**Selected impact:** USD 30M+ efficiencies delivered in eight months | Enterprise workforce plan across 3,800+ employees and 14+ locations | Technology, cloud and vendor cost transparency | VMO and managed-services governance

* Developed the first enterprise-wide Technology workforce plan, covering approximately **3,800 employees across 14+ locations** and addressing location strategy, central versus local responsibilities, future skills, access to talent and evolving headcount requirements.
* Established workforce mapping, tracking and governance across the technology organisation, improving leadership visibility of roles, reporting lines, locations, skills, sourcing models and organisational structure.
* Designed a total cost of technology model and planning process, bringing together workforce effort, application costs, vendors, cloud, third-party services and allocations into a consolidated leadership view.
* Built detailed cloud cost and structural views, identifying cost drivers, allocation challenges and opportunities for optimisation and improved accountability.
* Developed TCO, TCOA, cost-to-serve, productivity and performance measures to support better decisions on resource deployment, sourcing, investment and operational efficiency.
* Launched the Technology APEX efficiency framework, delivering more than **USD 30M in savings within eight months** across eight optimisation pillars.
* Established a Vendor Management Office and governance framework, improving strategic oversight, simplifying the vendor landscape and strengthening supplier performance, risk and commercial discipline.
* Designed and implemented an outcome-based managed-services approach to improve accountability for delivery, service quality and value.
* Established a preferred-vendor framework for contingent workers, improving compliance, consistency, cost control and access to capability.
* Led the Technology and Operations Learning \& Development team, aligning capability-building activity with business priorities, future skills and the practical adoption of AI tools.
* Supported the development of an AI-enabled workforce strategy, considering how automation, new skills and changing work practices could affect future capability, operating models and workforce demand.
* Led **nine direct reports across four countries**, using co-created objectives, clear accountability, coaching and empathetic leadership to build a collaborative, high-performing team.
* Supported the Technology global team through a Groupwide Audit and supported other Audit requirements.

\---

### Standard Chartered Bank, Singapore — 2016 to 2024

#### KPI \& Metrics Lead / People \& Culture Lead, CPBB Technology — Apr 2020 to Jan 2024

Led performance insight, workforce, culture, capability and executive reporting activity across a **4,000+ person retail technology organisation**.

* Designed and sourced multiple data streams to create a 360-degree view of organisational health, performance, workforce and delivery.
* Created and delivered a KPI and metrics suite for senior leadership, improving transparency, accountability and data-led decision-making.
* Built RAG reporting, executive commentary and **Paths to Green**, helping leaders identify risks, actions and improvement priorities.
* Developed cost-of-technology, TCOA, cost-to-serve, productivity and organisational-health methodologies and KPIs for retail technology.
* Built and rolled out **Four Pillars**, a People, Culture, Learning \& Engagement framework for 4,000+ employees.
* Partnered on organisation redesign, manpower planning, job evaluation, skills and capability activity.
* Implemented hybrid-working practices through **Team Agreements**, clarifying expectations, collaboration norms and accountability.
* Designed and delivered large-scale mentoring, coaching and leadership-development programmes.
* Created and ran multi-day learning events under the **Learning Is My Day Job** programme.
* Developed **Speaking of Success**, a leadership-content series sharing senior leaders' career journeys and lessons.
* Advised executives on workforce, culture, capability and organisational strategy.
* Partnered with workplace and real-estate teams to align office design with collaboration, learning, culture and well-being.
* Designed and facilitated leadership communications, town halls, learning sessions, recognition events and award programmes.

#### Head of Business Planning \& Strategy / Head of PMO, T\&I — Financial Markets — May 2016 to Apr 2020

Led business planning, PMO, portfolio governance, workforce planning and transformation support across Financial Markets Technology.

* Directed governance across a **USD 60M+ project and BAU portfolio**.
* Built and managed a PMO team of **five specialists** supporting change and BAU cost analysis, tracking, governance and implementation.
* Oversaw workforce planning for approximately **1,200 employees** across Singapore, London and Chennai.
* Introduced portfolio dashboards and executive-level reporting, improving transparency of delivery, cost, risks and priorities.
* Strengthened project-versus-BAU cost and resource visibility to support planning and investment decisions.
* Streamlined vendor and contractor management processes.
* Supported major transformation rollouts, including North Star design, new ways of working and organisational values.
* Led People \& Culture activity across multi-market teams through learning, engagement, recognition and the **People Matter** programme.
* Managed promotion planning, succession planning and talent-review processes and workshops.
* Supported workplace planning, office changes and leadership communications across the regional organisation.

\---

### Bank of America Merrill Lynch, Singapore — May 2010 to May 2016

**VP, Business Manager, FICC Technology — APAC \& Global Emerging Markets**

Led regional technology business management across financial governance, workforce planning, executive reporting, operating support and organisational initiatives.

* Managed a **USD 40M annual technology budget** and **USD 80M application cost base**.
* Redesigned and rebalanced Cost of Application reporting and allocations, improving transparency and accountability.
* Directed workforce planning, headcount management and location strategy across APAC.
* Produced ExCo management packs covering financials, risk, delivery, workforce and organisational priorities.
* Supported regulatory programmes and infrastructure-resilience initiatives.
* Partnered with senior leaders on cost, resourcing, operating-model and organisational decisions.
* Led employee engagement, leadership communication and regional culture initiatives.
* Managed promotion planning, succession planning and talent-review processes and workshops.
* Supported major office restacks and workplace changes across the APAC organisation.

\---

### Royal Bank of Scotland, Singapore — Nov 2009 to Apr 2010

**Asia MI Contractor**

* Produced management information reporting and analytics across Asia.
* Delivered financial packs supporting workforce, portfolio and cost planning.
* Partnered with technology leaders to improve transparency of spend.
* Redesigned reporting processes and outputs, improving quality and speed of delivery.

\---

### American Express, Singapore \& UK — 1998 to 2009

#### Asia/Australia Human Resources — Manager, Budget \& Project Planning — Nov 2007 to Mar 2009

* Managed and forecasted a regional HR budget across **14 cost centres totalling approximately USD 12M+**.
* Directed headcount reporting, cost allocation, employee-survey and workforce analysis.
* Achieved the 2008 regional budget target.
* Implemented diversity reporting for **13,000+ employees**.
* Negotiated 2009 regional budgets during a period of organisational restructuring.
* Developed a time-allocation methodology to charge HR costs across 13 business lines.
* Supported promotion planning, succession planning and talent-review processes and workshops.

#### Membership Travel Services International — Manager, Business Analysis \& Strategic Partnerships — Aug 2005 to Nov 2007

* Owned and managed the global Membership Travel MI structure.
* Implemented the first travel-activity reporting tool at zero additional cost by leveraging existing corporate MI capability.
* Designed, developed and managed data feeds into MI reporting systems.
* Rolled out MI systems and trained **50+ users** across regions.
* Integrated Mexico, Thailand and Taiwan into the global MI database at zero additional cost.
* Supported supplier negotiations and preferred-partner programmes through data and commercial insight.
* Contributed to **Project Freedom**, enabling Membership Rewards points to be used as travel payment.
* Acted as the travel-data SME supporting partner negotiations and consumer-travel discussions.

#### Membership Rewards Global Reporting Finance — Senior Analyst — Oct 2003 to Aug 2005

* Built complex Excel models for Membership Rewards liability, value and customer reporting.
* Delivered financial analysis and management information.
* Partnered with Technology teams on process improvement and data accuracy.
* Designed and implemented SOX compliance tracking and reporting.

#### Innovation \& Development — Travel Consultant / Analyst, MI Product Management — Oct 2002 to Oct 2003

* Produced customised travel MI reports for global customers.
* Oversaw UAT, documentation and process improvements.
* Supported Marketing with MI-driven insight.
* Contributed to the development of the Innovation \& Development function.

#### Sales Planning \& Pricing Technology — Research Analyst — Aug 2000 to Oct 2002

* Operated the MI REPS reporting helpdesk.
* Delivered UAT, error analysis and user training.
* Managed system expansion and improvement projects.
* Served as database administrator for the REPS system.

#### Corporate Travel Operations — European Travel MI Consultant — Aug 1999 to Aug 2000

* Delivered database administration, MI reporting and customer support for European travel operations.
* Built and implemented a report-tracking and productivity database to replace a non-compliant system.
* Served as a travel-data SME.

#### Travel MI Operations — MI Officer, Europe — Aug 1998 to Aug 1999

* Delivered MI reporting and analysis across European travel operations.
* Served as a travel-data SME.

\---

<a id="core-skills-and-capabilities"></a>

## 🎯 Core Skills \& Capabilities

### Executive Leadership \& Business Management

* COO, CIO and CTOO office support
* Strategy execution and executive decision support
* Governance forums, operating rhythms and management reporting
* Regional and global team leadership
* Executive communication, presentations and storytelling
* Trusted-advisor support in complex, matrixed organisations

### PMO, Transformation \& Continuous Improvement

* PMO and portfolio leadership
* Programme governance, prioritisation and delivery oversight
* Project-versus-BAU transparency
* Risk, issue, dependency and action management
* Operating-model transformation
* Process redesign, standardisation and workflow improvement
* Regulatory and risk-driven programme support
* Benefits, value and performance tracking
* Six Sigma process-improvement methods

### Workforce \& Organisation Strategy

* Strategic workforce planning
* Headcount, location and sourcing strategy
* Organisation design and role mapping
* Future skills and capability planning
* AI-enabled workforce transformation
* Succession, talent and promotion planning
* Workforce analytics and scenario-based decision support

### Financial, Cost \& Performance Management

* Technology finance and business management
* Total cost of technology and TCOA
* Cloud cost and structure analysis
* Cost-to-serve and productivity methodologies
* Budgeting, forecasting and variance analysis
* Portfolio and application cost management
* Cost-efficiency programmes
* KPI, scorecard and executive-performance frameworks

### Vendor, Sourcing \& Partnerships

* Vendor Management Office design
* Vendor governance and convergence
* Preferred-vendor frameworks
* Outcome-based managed services
* Contingent workforce governance
* Supplier-performance oversight
* Data and insight supporting supplier negotiations

### People, Culture, Learning \& Capability

* Learning and capability strategy
* AI-tool adoption and upskilling
* Mentoring and coaching programmes
* Leadership development and communication
* Culture and engagement frameworks
* Hybrid-working practices
* Employee experience, recognition and well-being
* Large-scale learning events and town halls

### AI \& Digital Innovation

* AI-enabled application and workflow design
* Agent and specialist-persona development
* Prompt engineering and structured prompt systems
* AI-assisted prototyping, testing and iteration
* Reusable prompt libraries and workflow templates
* Human-centred design
* Practical application of AI to research, analysis and productivity

### Data, Analytics \& Technology

* Advanced Excel modelling and MI reporting
* Data sourcing, consolidation and quality improvement
* Executive dashboards and RAG reporting
* Database administration and MI systems
* UAT, testing, troubleshooting and error analysis
* Data analysis across HR, Finance, Technology, Travel and Rewards

### Workplace \& Employee Experience

* Workplace strategy and space planning
* Office relocations, redesigns and restacks
* Workplace vendor and contractor engagement
* Employee-experience programming
* Health, safety and operational awareness

\---

<a id="education-and-professional-development"></a>

## 🎓 Education \& Professional Development

### Formal Education

* **MBA in Strategic Management** — Apsley Business School, 2024  
*Dissertation: AI and the Future of People \& Culture in Financial Services Technology*
* **Management Strategy MBA Module** — Open University Business School, 2006
* **Postgraduate Diploma of Management** — Open University Business School, 2006
* **Professional Certificate of Management** — Open University Business School, 2005
* **ISS International School of Singapore** — 7 GCSEs and 3 A Levels

### Selected Professional Development

* LEAP Leadership — Prudential
* LUMA Human-Centred Design Practitioner
* Shackleton Leadership Experience
* Bank of America Global Leadership
* Six Sigma Green and Yellow Belt
* SOX 404 Compliance
* American Express Project Management
* Authentic Leadership
* Situational Leadership II
* Executive Presence
* Employee Engagement \& HR Leadership
* Public Speaking, Toastmasters and Storytelling
* Chief of Staff development
* Business-focused PMO delivery
* Mentoring-programme development
* Thought leadership
* Communicating about culturally sensitive issues
* Compliance training across American Express, Bank of America, Standard Chartered and Prudential
* Fire Warden and AED training

\---

<a id="about-me"></a>

## About Me

Naturally curious, analytical and interested in how people, organisations, technology and markets work. Outside work, I enjoy long-distance hiking, running, pub quizzes, reading, metal detecting and family life.

I stay actively engaged with current affairs, politics, geopolitics, technology, AI, investment markets and workforce trends. These interests reinforce the qualities I bring professionally: preparation, curiosity, pattern recognition, persistence and a willingness to examine problems from several perspectives.

